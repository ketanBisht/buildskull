...
function FAQItem({ q, a, i }: { q: string; a: string; i: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className={`text-xl md:text-2xl font-black transition-colors duration-300 ${open ? 'text-primary' : 'text-black/80 group-hover:text-black'}`}>
          {q}
        </span>
        <div className={`w-10 h-10 rounded-full border flex items-center justify-center flex-shrink-0 transition-all duration-300 ${open ? 'bg-primary border-primary text-white' : 'bg-black/[0.02] border-black/[0.04] text-black/20 group-hover:border-black/10'}`}>
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <p className="pb-8 text-lg text-black/60 font-medium leading-relaxed max-w-3xl">
              {a}
            </p>
          </motion.div>
        )}
...
export default function FAQSection() {
  ...
  return (
    <section id="faq" ref={ref} className="py-32 px-6 bg-white relative overflow-hidden">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-[#fcfcfc] rounded-[48px] border border-black/[0.04] shadow-apple p-8 md:p-12"
        >
          {faqs.map((faq, i) => (
        ...
        >
          <p className="text-black/40 font-bold">
            Still curious? {' '}
            <a href="#contact" className="text-primary hover:underline underline-offset-4 transition-all">
